...
def is_contain_Arabic(check_str):
    for ch in check_str:
        if u'\u0600' <= ch <= u'\u06ff' or u'\u0750' <= ch <= u'\u077f' or u'\ufb50' <= ch <= u'\ufdff' or u'\ufe70' <= ch <= u'\ufeff':
            return True
    return False

def is_contain_Bangla(check_str):
    for ch in check_str:
        if u'\u0980' <= ch <= u'\u09ff':
            return True
    return False

def is_contain_Japanese(check_str):
    for ch in check_str:
        if u'\u3040' <= ch <= u'\u30ff' or u'\u31f0' <= ch <= u'\u31ff':
            return True
    return False

def is_contain_Korean(check_str):
    for ch in check_str:
        if u'\u1100' <= ch <= u'\u11ff' or u'\u3130' <= ch <= u'\u318f' or u'\uac00' <= ch <= u'\ud7af':
            return True
    return False

#dwaw texts of mutil language
def draw_texts_of_ML(img, texts, boxes=None, language=None):

    font_path = '/workspace/str/fonts/' + language + '.ttf' 
    #print(font_path)
    color_list = gen_color()
    h, w = img.shape[:2]
    if boxes is None:
...
def det_recog_show_result(img, end2end_res):
    ...
    elif is_contain_Korean(''.join(texts)):
        text_vis_img = draw_texts_of_ML(img, texts, boxes,'Korean')
    elif is_contain_Arabic(''.join(texts)):
        text_vis_img = draw_texts_of_ML(img, texts, boxes,'Arabic')
    elif is_contain_Bangla(''.join(texts)):
        text_vis_img = draw_texts_of_ML(img, texts, boxes,'Bangla')
    elif is_contain_Japanese(''.join(texts)):
        text_vis_img = draw_texts_of_ML(img, texts, boxes,'Japanese')
    #default： English and symbols
    else:
        text_vis_img = draw_texts(img, boxes, texts)

    #text_vis_img = draw_texts_of_ML(img, texts, boxes,'unifont')
